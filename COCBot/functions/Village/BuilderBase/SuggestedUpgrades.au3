...
; MAIN CODE
Func MainSuggestedUpgradeCode($bDebugImage = $g_bDebugImageSave, $bFinishNow = False, $bTest = False)
	...
	If $g_iBuilderHallLevel = 0 Or  $g_iBuilderHallLevel >= 8 Then
					...
					Switch $aResult[2]
						...
						Case "New"
							If $g_iChkPlacingNewBuildings = 1 Then
								...
								If NewBuildings($aResult, $bDebugImage, $bTest) Then
									ExitLoop
								EndIf
								$bSkipGoldCheck = True
							Else
...
Func NewBuildings($aResult, $bDebugImage = $g_bDebugImageSave, $bTest = False)
			...
			For $i = 0 To UBound($ClocksCoordinates) - 1
				...
				If isArray($aiUpgZero) And Ubound($aiUpgZero) = 2 Then
					...
					Else
						...
						Local $aSearchDiamond = $CoCDiamondDCD ;GetReduceDiamond(30)
						...
						If IsArray($aiYesCoord) And UBound($aiYesCoord) = 2 Then
							...
							If Not $bTest Then ClickP($aiYesCoord)
							...
							If IsArray($aiNoCoord) And UBound($aiNoCoord) = 2 Then
								...
								If Not $bTest Then 
									SetLog("Found another building!", $COLOR_WARNING)
								Else
									SetLog("New Building placement test completed!", $COLOR_INFO)				
									If $bDebugImage Then SaveDebugPointImage("NewBuildingPlacement", $aiYesCoord)	
								EndIf
