	...
	While 1
		...
		Switch $g_aUpgradeNameLevel[1]
			Case "Barbarian King", "Archer Queen", "Grand Warden", "Royal Champion"
				...
				$g_aUpgradeResourceCostDuration[1] = getUpgradeCost(598, 521 + $g_iMidOffsetY) ; get cost
			...
			Case Else
				...
				$g_aUpgradeResourceCostDuration[1] = getUpgradeCost(361, 485 + $g_iMidOffsetY) ; get cost
...
Func getUpgradeCost($iX1, $iY1)

	Local $iCost = 0
	Local $sCost = ""
	Local $iX2 = $iX1 + 106
	Local $iY2 = $iY1 + 22
	Local $sSearchArea = GetDiamondFromRect2($iX1,$iY1,$iX2,$iY2)
	Local $sImgWhiteZero = @ScriptDir & "\imgxml\Resources\UpgradeCost\"
	Local $avZero = findMultiple($sImgWhiteZero, $sSearchArea, $sSearchArea, 0, 1000, 1, "objectname,objectpoints", True)

	SaveDebugDiamondImage("getUpgradeCost", $sSearchArea)

	If Not IsArray($avZero) Or UBound($avZero, $UBOUND_ROWS) <= 0 Then
		SaveImageTemplate("FailedZero")
		SetLog("Failed to detect numbers", $COLOR_ERROR)
		Return $iCost
	EndIf

	Local $avTempArray, $aiZeroCoords
	For $i = 0 To UBound($avZero, $UBOUND_ROWS) - 1
		$avTempArray = $avZero[$i]
		If StringInStr($avTempArray[0], "1", $STR_NOCASESENSE) Then
			$sCost = getUpgradeCost1($iX1, $iY1)
			SetLog("Detected Large Zero : " & $sCost, $COLOR_INFO)
			$iCost = number(StringStripWS($sCost,7))
			SaveImageTemplate("LargeZero")
			SetLog("Detected Large Zero : " & $iCost, $COLOR_INFO)
		ElseIf StringInStr($avTempArray[0], "2", $STR_NOCASESENSE) Then
			$sCost = getUpgradeCost2($iX1, $iY1)
			$iCost = number(StringStripWS($sCost,7))
			SaveImageTemplate("MediumZero")
			SetLog("Detected Medium Zero : " & $iCost, $COLOR_INFO)
		ElseIf StringInStr($avTempArray[0], "3", $STR_NOCASESENSE) Then
			$sCost = getUpgradeCost3($iX1, $iY1)
			$iCost = number(StringStripWS($sCost,7))
			SaveImageTemplate("SmallZero")
			SetLog("Detected Small Zero : " & $iCost, $COLOR_INFO)
		Else
			SetLog("Detected : " & $avTempArray[0] & "!", $COLOR_WARNING)
			SaveImageTemplate("UnknownZero")
		EndIf
	Next
