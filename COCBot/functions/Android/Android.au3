	...
	If Not $g_bAndroidInitialized Or $g_iAndroidAdbProcess[0] = 0 Or ProcessExists2($g_iAndroidAdbProcess[0]) <> $g_iAndroidAdbProcess[0] Then
		...
		Switch $g_iAndroidVersionAPI
			...
			Case $g_iAndroidPie
				...
				EndIf
			Case Else
				SetLog("Android Version not detected!")
		EndSwitch
		...
		If $g_iAndroidAdbMinitouchMode = 0 Then
			If $g_bAndroidAdbMinitouchPort Then
				SetLog($g_sAndroidEmulator & " initialize minitouch on port " & $g_bAndroidAdbMinitouchPort)
				; launch minitouch
	...
	Else
		...
		If $bIsMinitouch Then
			If $g_iAndroidVersionAPI = $g_iAndroidPie And $g_sAndroidEmulator = "Memu" Then
				...
				SetLog("Pie : " & """" & $minitouchPath & "minitouch"" -v -d " & $g_sAndroidMouseDevice & " -f """ & $androidPath & $scriptFileSh & """")
			Else
...
Func GetAndroidProcessPID($sPackage = Default, $bForeground = True, $iRetryCount = 0)
	...
	Local $cmd = "set result=$(ps -A -o USER,PID,SCHED,NAME|grep """ & $g_sAndroidGamePackage & """ >&2)"
	...
	If $error = 0 Then
		...
		For $i = 1 To UBound($aPkgList)
			...
			If $iCols = 4 And $aPkgList[$i - 1][$iCols - 1] = $g_sAndroidGamePackage Then
				; process running
				If $bForeground = True And $aPkgList[$i - 1][2] <> $CorrectSCHED Then
					; not foreground
